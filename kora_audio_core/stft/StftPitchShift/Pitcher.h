...
namespace stftpitchshift
{
  ...
  public:
    ...
    void shiftpitch(std::vector<std::complex<T>>& dft, float *progress)
    {
      if (values.empty())
      {
        return;
      ...
      float progress_max = values.size() == 1 ? dft.size() : (values.size() * dft.size()) + (dft.size() * values.size());
      float progress_actual = 0;
      ...
      if (values.size() == 1)
      {
        ...
        {
            *progress = (++progress_actual) / progress_max;
