...
namespace stftpitchshift
{
  ...
  public:
    ...
    void operator()(const std::vector<T>& input, std::vector<T>& output, const std::function<void(std::vector<std::complex<T>>& dft)> callback, float *progress)
    {
      (*this)(input.size(), input.data(), output.data(), callback, progress);
    }

    void operator()(const size_t size, const T* input, T* const output, const std::function<void(std::vector<std::complex<T>>& dft)> callback, float *progress)
    {
