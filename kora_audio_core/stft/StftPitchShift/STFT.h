...
namespace stftpitchshift
{
  ...
  public:
    ...
    void operator()(const std::vector<T>& input, std::vector<T>& output, const std::function<void(std::vector<std::complex<T>>& dft)> callback, float *progress) const
    {
      (*this)(input.size(), input.data(), output.data(), callback, progress);
    }

    void operator()(const size_t size, const T* input, T* const output, const std::function<void(std::vector<std::complex<T>>& dft)> callback, float *progress) const
    {
      ...
      if (chronometry)
      {
        ...
        timers.loop.tic();
          size_t hop = 0;
          for (hop = 0; (hop + framesize) < size; hop += hopsize)
          {
              ++prog_max;
          }
        for (hop = 0; (hop + framesize) < size; hop += hopsize)
        {
            *progress = (++prog) / prog_max;
          timers.analysis.tic();
