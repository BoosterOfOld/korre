  ...
  float *progress)
{
  ...
  if (hopsize == 1)
  {
    ...
    sdft(size, input, output, [&, progress](std::vector<std::complex<float>>& dft)
    {
      core.shiftpitch(dft, &progx);
    }, progress);
  }
  else
  {
    ...
    stft(size, input, output, [&, progress](std::vector<std::complex<float>>& dft)
    {
      core.shiftpitch(dft, &progx);
    }, progress);
  }
